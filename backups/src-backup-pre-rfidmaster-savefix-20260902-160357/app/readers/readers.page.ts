...
export class ReadersPage implements OnDestroy {
  ...
  ionViewDidEnter() {
    ...
    if (this.subs.length === 0) {
      this.subs.push(
        ...
        this.hardwareRfid.pairing$.subscribe((ev) => {
          this.pairingStatus = ev.message || ev.status || '';
        }),
        this.hardwareRfid.pairingComplete$.subscribe(() => {
          this.pairing = false;
          this.pairingStatus = '';
          this.refresh();
        })
  ...
  async pairNew() {
    ...
    this.pairing = true;
    this.pairingStatus = 'Searching for RFD4031 reader...';
    try {
      const result = await this.hardwareRfid.pairReader();
      if (result.paired) {
        this.toast.success('Reader paired: ' + (result.name || 'RFD4031'));
      }
    } catch (err) {
      console.error('[Readers] pairing failed:', err);
      this.toast.danger('Could not pair the new reader');
    } finally {
      this.pairing = false;
      this.pairingStatus = '';
      this.refresh();
