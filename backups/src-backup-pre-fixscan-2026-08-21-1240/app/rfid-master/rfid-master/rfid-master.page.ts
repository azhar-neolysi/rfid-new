...
export class RfidMasterPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subs.push(
      ...
      this.hardwareRfid.tagDetails$.subscribe((event) => {
        if (!this.pageActive) return;
        this.onTagDetails(event);
      })
    );
  ...
  onTagDetails(event: { epc: string; tid: string; userMemory: string }) {
    if (!this.currentScanEpc || event.epc !== this.currentScanEpc) return;
    if (event.tid) {
      this.rfidForm.controls.systemId.setValue(event.tid);
      this.rfidForm.controls.sysMemoryId.setValue(tidChipFamily(event.tid));
    }
    if (event.userMemory) {
      this.rfidForm.controls.userMemoryId.setValue(event.userMemory);
      const words = Math.floor(event.userMemory.length / 4);
      if (words > 0) {
  ...
  async showDuplicateToast(record: any) {
    const t = await this.toast.create({
      color: 'warning',
      message: 'Tag already exists — loaded for editing',
      position: 'top',
      duration: 2000,
    });
    t.present();
