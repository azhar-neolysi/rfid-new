...
export class TagingPage implements OnInit, OnDestroy {
  ...
  search(event: any) {
    if (event.target.value) {
      this.navigateToProduct(event.target.value);
    }
  }
  ...
  private navigateToProduct(code: string) {
    const data = { barcode: code, rfidcode: code };
    this.product.searchProduct(data).subscribe({
      next: (res: any) => {
        const found =
          res &&
          ((res.productEntry && res.productEntry.length > 0) ||
            res.productEntryId);
        if (found) {
          this.router.navigate(['itemmaster', code]);
        } else {
          this.toast.danger('No product found for this barcode/tag');
        }
      },
      error: () => {
        this.toast.danger('Unable to look up product');
      },
    });
  }
}
