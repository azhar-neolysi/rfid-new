    ...
    if (this.referenceForm.valid) {
      if (this.refId) {
        ...
        this.reference.getReference().subscribe({
          next: (list: any[]) => {
            const dup = (list || []).some(
              (r) =>
                ...
                String(this.referenceForm.value.refName || '').trim().toLowerCase()
            );
            if (dup) {
              this.toast.danger('Reference name already exists');
              return;
            }
            const data = {
              name: this.referenceForm.value.refName,
          ...
          error: () => this.toast.danger('Failed to validate reference name'),
        });
      }
    } else {
